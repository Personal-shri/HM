...
if(researchTopics.length){
 ...
 search.addEventListener('input',()=>{const q=search.value.trim().toLowerCase();let count=0;researchTopics.forEach(topic=>{topic.hidden=!!q&&!topic.textContent.toLowerCase().includes(q);if(!topic.hidden){count++;if(q)topic.open=true;}});document.getElementById('research-count').textContent=count+' of '+researchTopics.length+' research topics';document.getElementById('research-empty').hidden=count!==0;});
