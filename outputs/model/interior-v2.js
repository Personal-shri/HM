...
function go(index){closePhoto();wall.clippingPlanes=[];labelGroup.visible=false;current=index;if(window.HMPhotoReady)syncReference();mode='walk';orbit.enabled=false;const [name,x,y,angle]=rooms[index];camera.position.set(x-22,5.2,24-y);yaw=angle;pitch=0;orient();status.textContent=name+' · Walk mode · drag to look, use arrows or WASD to move';document.querySelector('#mode').textContent='Overview';document.querySelector('#room').value=index;drawMap();}
