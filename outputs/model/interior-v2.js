...
function go(index){closePhoto();renderer.clippingPlanes=[];document.querySelector('#isolate-room').checked=false;wall.clippingPlanes=[];labelGroup.visible=false;current=index;if(window.HMPhotoReady)syncReference();mode='walk';orbit.enabled=false;const [name,x,y,angle]=rooms[index];camera.position.set(x-22,5.2,24-y);yaw=angle;pitch=0;orient();status.textContent=name+' · Walk mode · drag to look, use arrows or WASD to move';document.querySelector('#mode').textContent='Overview';document.querySelector('#room').value=index;drawMap();}
...
document.querySelector('#mode').onclick=()=>{renderer.clippingPlanes=[];document.querySelector('#isolate-room').checked=false;if(mode==='walk'){mode='overview';wall.clippingPlanes=document.querySelector('#cutaway').checked?[new THREE.Plane(new THREE.Vector3(0,-1,0),3)]:[];labelGroup.visible=document.querySelector('#labels').checked;orbit.enabled=true;const fit=Math.max(1,1.15/camera.aspect);camera.position.set(-33*fit,46*fit,48*fit);orbit.target.set(0,0,0);orbit.update();status.textContent='Overview · drag to orbit; scroll or pinch to zoom · choose a room to walk';document.querySelector('#mode').textContent='Return to walking';}else go(current);};
...
document.querySelector('#focus-room').onclick=()=>{closePhoto();mode='overview';orbit.enabled=true;wall.clippingPlanes=[new THREE.Plane(new THREE.Vector3(0,-1,0),3)];labelGroup.visible=false;const r=rooms[current];orbit.target.set(r[1]-22,1,24-r[2]);const f=Math.max(1,1/camera.aspect);camera.position.copy(orbit.target).add(new THREE.Vector3(-12*f,19*f,17*f));orbit.update();status.textContent=r[0]+' · close overhead view; drag to rotate';document.querySelector('#mode').textContent='Return to walking';document.querySelector('#cutaway').checked=true;if(window.HMFocusReady)applyRoomFocus();};
document.querySelector('#mode').click();

const photoRooms=[
 ['living','Living / TV',1,'24 × 20 ft living zone, including the foyer notch. Sofa and TV positions follow furniture revision 03.'],
 ['kitchen-dining','Kitchen & dining',2,'Kitchen: 20 × 14 ft. Dining planning zone: 14 × 14 ft. Inspect the island, counters and table in the model.'],
 ['parents','Parents’ bedroom',4,'16 × 16 ft room. Compare the bed, bedside tables and clear walking routes with the model.'],
 ['guest','Guest bedroom',5,'14 × 14 ft room. The model retains the planned bed and wardrobe footprints.'],
 ['laundry-store','Laundry & storeroom',6,'Laundry: 6 × 8 ft; store: 6 × 6 ft. Finish study: the photo’s door position differs from the plan.'],
 ['bathrooms','Bathrooms',9,'Parents’ bath: 10 × 6 ft; shared bath: 8 × 6 ft. Fixture layouts in the image are proposals; model zones remain unfitted.'],
 ['foyer-hall','Foyer & hall',0,'Entry and circulation finish study. The photo’s openings and stairs are illustrative; follow the model for location.'],
 ['pantry-closet','Pantry & closet',8,'Two separate 6 × 6 ft spaces, shown together as a storage finish reference.']
];
let activePhoto=6;
function syncReference(){const key=photoMap[current];const index=photoRooms.findIndex(r=>r[0]===key);const img=document.querySelector('#reference-image');const expand=document.querySelector('#reference-expand');const download=document.querySelector('#reference-download');if(index<0){img.hidden=true;expand.hidden=true;download.hidden=true;document.querySelector('#reference-title').textContent=rooms[current][0];document.querySelector('#reference-description').textContent='No coordinated photo for the stair yet. Explore the reserved footprint in the model.';}else{activePhoto=index;const [key,title,,description]=photoRooms[index];img.hidden=false;expand.hidden=false;download.hidden=false;img.src=window.HMInteriorPhotos[key];img.alt=title+' saved interior design';document.querySelector('#reference-title').textContent=title;document.querySelector('#reference-description').textContent=description;download.href=img.src;download.download='HM-'+key+'.png';}document.querySelectorAll('.photo-card').forEach((b,i)=>b.setAttribute('aria-pressed',i===index?'true':'false'));}
photoRooms.forEach(([key,title,room],i)=>{const b=document.createElement('button');b.className='photo-card';const img=document.createElement('img');img.src=window.HMInteriorPhotos[key];img.alt=title+' design sample';const label=document.createElement('span');label.textContent=title;b.append(img,label);b.onclick=()=>{go(room);document.querySelector('.explorer').scrollIntoView({behavior:'smooth',block:'start'});};document.querySelector('#room-gallery').append(b);});
document.querySelector('#reference-expand').onclick=()=>document.querySelector('#photo').click();
for(const [id,step] of [['#photo-prev',-1],['#photo-next',1]])document.querySelector(id).onclick=()=>{const next=(activePhoto+step+photoRooms.length)%photoRooms.length;go(photoRooms[next][2]);document.querySelector('#photo').click();};
window.HMPhotoReady=true;syncReference();

// Nominal room boundaries, in the same feet coordinate system as the floor plan.
const roomBounds=[[18,0,24,6],[0,0,24,20],[24,0,44,14],[24,14,38,28],[0,26,16,42],[30,34,44,48],[38,20,44,28],[38,28,44,34],[38,14,44,20],[0,42,10,48],[10,42,16,48],[30,28,38,34],[16,42,30,48],[16,26,26,42]];
function applyRoomFocus(){const [x0,y0,x1,y1]=roomBounds[current];orbit.target.set((x0+x1)/2-22,1,24-(y0+y1)/2);const span=Math.max(x1-x0,(y1-y0),8);const fit=Math.max(1,1/camera.aspect);camera.position.copy(orbit.target).add(new THREE.Vector3(-.52*span*fit,1.03*span*fit,.94*span*fit));orbit.update();renderer.clippingPlanes=document.querySelector('#isolate-room').checked?[
 new THREE.Plane(new THREE.Vector3(1,0,0),22-x0+.2),new THREE.Plane(new THREE.Vector3(-1,0,0),x1-22+.2),
 new THREE.Plane(new THREE.Vector3(0,0,1),y1-24+.2),new THREE.Plane(new THREE.Vector3(0,0,-1),24-y0+.2)]:[];}
document.querySelector('#isolate-room').onchange=()=>document.querySelector('#focus-room').click();
function zoomModel(factor){if(mode==='walk'){document.querySelector('#focus-room').click();}const offset=camera.position.clone().sub(orbit.target);const distance=Math.max(2.5,Math.min(150,offset.length()*factor));camera.position.copy(orbit.target).add(offset.setLength(distance));orbit.update();}
document.querySelector('#zoom-in').onclick=()=>zoomModel(.75);document.querySelector('#zoom-out').onclick=()=>zoomModel(1.33);
document.querySelector('#fit-house').onclick=()=>{if(mode==='overview')go(current);document.querySelector('#mode').click();};
const ray=new THREE.Raycaster();renderer.domElement.addEventListener('dblclick',e=>{if(mode!=='overview')return;const rect=renderer.domElement.getBoundingClientRect();ray.setFromCamera(new THREE.Vector2((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1),camera);const point=new THREE.Vector3();if(ray.ray.intersectPlane(new THREE.Plane(new THREE.Vector3(0,1,0),0),point)){point.x=THREE.MathUtils.clamp(point.x,-22,22);point.z=THREE.MathUtils.clamp(point.z,-24,24);const offset=camera.position.clone().sub(orbit.target).multiplyScalar(.55);orbit.target.copy(point);camera.position.copy(point).add(offset);orbit.update();}});
document.querySelector('#fullscreen').onclick=async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await host.requestFullscreen();}catch{status.textContent='Fullscreen is unavailable in this browser. The full-width model and zoom controls remain available.';}};
document.addEventListener('fullscreenchange',()=>{document.querySelector('#fullscreen').textContent=document.fullscreenElement?'Exit fullscreen ⛶':'Fullscreen ⛶';resize();});
window.HMFocusReady=true;
