...
source=source.replace("'DOORS, LARGE WINDOWS & DIMENSIONS'","'MAIN FLOOR — TV, DAYLIGHT & FURNITURE'").replace("'D1: 4 × 8 ft entry • D2: 3.5 × 7 ft • D3: 3 × 7 ft • D4: 2.5 × 7 ft store slider'","'Existing room boundaries and opening coordinates retained • front / street at bottom'").replace("'W1: 8 × 6 ft • W2: 10 × 4 ft • W3: 6 × 4 ft • W4: 4 × 4 ft • V1: 2 × 2 ft • S1: 7 × 8 ft'","'Furniture drawn to the same scale • nominal zones, not finished clear dimensions'")
source=source.replace("'american-dimensioned.svg'","'main-floor-furniture-v3.svg'")
insert='''
 ...
 item(.5,7.5,1.25,6,'TV')
 item(9.5,6.5,3,7.5,'L1')
 item(3.5,16,6,3,'L2')
 item(5.5,9,2,3.5,'L3')
 item(23,1,1,3,'F1')
 ...
 for x,y,label in [(6,4.5,'LIVING 24 × 20*'),(34,12.5,'KITCHEN 20 × 14'),(32,25,'DINING 14 × 14*'),(7,28,'PARENTS 16 × 16'),(37,46.5,'GUEST 14 × 14'),(5,46,'ENSUITE'),(12.8,47,'CLOSET'),(34,32,'BATH'),(41,29,'STORE'),(40,27,'LAUNDRY'),(40,19,'PANTRY')]:zone(x,y,label)
 # Nominal working-gap checks, excluding wall deductions.
 # TV screen and illustrative central seated sightline, not a sunlight simulation.
 wall(.55,8.1,.55,12.9)
 a,b=xy(.65,10.25);c,d=xy(11,10.25)
 P.append(f'<path d="M{a},{b} L{c},{d}" stroke="#486b9c" stroke-width="2" stroke-dasharray="4 4"/>')
 tag(5.7,7.5,'~10 ft eye-to-screen')
 tag(6,1.3,'BLIND + CURTAIN')
 tag(13.5,1.3,'SEPARATE CURTAIN')
 dimension(7.5,12,9.5,12,'2 ft')
...
"""
keys=[('L1','Primary sofa 7.5 × 3 ft; faces TV'),('L2','Conversation sofa 6 × 3 ft'),('L3','Coffee table 3.5 × 2 ft'),('TV','Console 6 × 1.25 ft; left wall'),('F1','Slim foyer cabinet 3 × 1 ft'),('K1/K2','Perimeter counters, 2 ft deep'),('K3','Fridge allowance 3 × 3 ft'),('K4','Island 6 × 3 ft; stools separate'),('T1','Dining table 6 × 3 ft, six seats'),('P1','Bed frame 7 × 5.5 ft; head right'),('P2','Bedside tables 1.5 ft square'),('P3','Chair 2.5 ft square'),('C1','Shelves 3.5 × 1.5 ft, right wall'),('G1','Bed frame 5.5 × 7 ft; head front'),('G2','Wardrobe 5 × 2 ft, left wall'),('G3','Bedside table 1.5 ft square'),('U1','Washer placeholder 2.33 × 2.5 ft'),('U2','Small sink 1.67 × 1.5 ft'),('ST1/ST2','Store shelves 1 ft deep'),('A1','Pantry shelves 1 ft deep')]
for i,(k,v) in enumerate(keys):footer+=f"txt(1345,{240+i*34},{(k+' — '+v)!r},15)\n"
notes=['Brown dashed line = circulation intent.', 'Blue dashed line = primary TV sightline.', 'Window shading is proposed, not sun-tested.','Blue lines = original windows / sliders.','D/S opening tags retain original meanings.','Furniture IDs are separate from door IDs.','No exterior window moved or added.','Bathroom fixtures not yet laid out.','Dining-side pantry/laundry door swings', 'require coordination before fixing furniture.', 'Schematic stair/access retained, not validated.','* Foyer/dining edges require final wall check.','Do not order furniture from nominal zones.']
for i,n in enumerate(notes):footer+=f"txt(1335,{950+i*26},{n!r},14)\n"
